...
const insertSchema = Joi.object({
    locality: Joi.string().min(3).max(20),
    phase: Joi.string().max(40),
    content: Joi.string().min(5).max(200),
    scientific_name: Joi.string().min(4).max(40).required(),
    born_captivity: Joi.bool().required(),
    price: Joi.number().precision(2).max(20000).required(),
    native_country: Joi.string().min(3).max(20).required(),
...
const updatedSchema = Joi.object({
    ...
    phase: Joi.string().max(40),
    content: Joi.string().min(5).max(200),
    scientific_name: Joi.string().min(4).max(40),
    born_captivity: Joi.bool(),
    price: Joi.number().precision(2).max(20000),
    native_country: Joi.string().min(3).max(20),
